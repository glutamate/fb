...
getAdCampaign (Id_ id) fl mtoken = getObject ("/v2.7/" <> id <> "/campaigns") [("fields", textListToBS $ fieldNameList $ Id ::: fl)] $ Just mtoken
...
setAdCampaign (Id_ id) r mtoken = postForm ("/v2.7/" <> id <> "/campaigns") (toForm r) mtoken
...
updAdCampaign (CreateCampaignId id) r mtoken = postForm ("/v2.7/" <> id <> "") (toForm r) mtoken
...
delAdCampaign :: (R.MonadResource m, MonadBaseControl IO m, AdCampaignDel r) =>
  ...
  ->  UserAccessToken -- ^ Optional user access token.
  -> FacebookT Auth m (Either FacebookException Success)
delAdCampaign (CreateCampaignId id) r mtoken = deleteForm ("/v2.7/" <> id <> "") (toForm r) mtoken
